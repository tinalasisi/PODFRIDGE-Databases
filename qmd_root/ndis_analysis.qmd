...
# Arrestee
p_arrestee_raw <- ndis_intermediate %>%
  plot_ly(x = ~capture_datetime, y = ~arrestee, color = ~jurisdiction, 
          type = 'scatter', mode = 'lines+markers', alpha = 0.7,
          name = ~jurisdiction) %>%
  layout(title = "Arrestee Profiles - Raw Data",
         xaxis = list(title = "Date and Time",
      tickformat = "%Y-%m-%d %H:%M"),
...
# Forensic
p_forensic_raw <- ndis_intermediate %>%
  plot_ly(x = ~capture_datetime, y = ~forensic_profiles, color = ~jurisdiction, 
          type = 'scatter', mode = 'lines+markers', alpha = 0.7,
          name = ~jurisdiction) %>%
  layout(title = "Forensic Profiles - Raw Data",
         xaxis = list(title = "Date and Time",
      tickformat = "%Y-%m-%d %H:%M"),
...
p_investigations_raw <- ndis_intermediate %>%
  plot_ly(x = ~capture_datetime, y = ~investigations_aided, color = ~jurisdiction, 
          type = 'scatter', mode = 'lines+markers', alpha = 0.7,
          name = ~jurisdiction) %>%
  layout(title = "Investigations Aided - Raw Data",
         xaxis = list(title = "Date and Time",
      tickformat = "%Y-%m-%d %H:%M"),
...
# NDIS Labs
p_labs_raw <- ndis_intermediate %>%
  plot_ly(x = ~capture_datetime, y = ~ndis_labs, color = ~jurisdiction, 
          type = 'scatter', mode = 'lines+markers', alpha = 0.7,
          name = ~jurisdiction) %>%
  layout(title = "NDIS Labs - Raw Data",
...
The stacked bar chart displays the temporal distribution of NDIS data snapshots, distinguishing between retained data (gray) and filtered records (red) for each time period. This visualization reveals when data quality issues were most prevalent and illustrates the impact of the cleaning process across different reporting periods, providing insight into the consistency and reliability of data submissions over time.

```{r}
#| label: stackedbar-coverage
#| echo: true
#| code-fold: true
...
stacked_bar_data <- ndis_data %>%
  mutate(year = year(capture_datetime)) %>%
  count(jurisdiction, year, name = "total_records") %>%
  left_join(
    ndis_clean %>%
      mutate(year = year(capture_datetime)) %>%
      count(jurisdiction, year, name = "retained_records"),
    by = c("jurisdiction", "year")
  ) %>%
  mutate(
    retained_records = replace_na(retained_records, 0),
    filtered_records = total_records - retained_records
  ) %>%
  pivot_longer(
    cols = c(retained_records, filtered_records),
    names_to = "record_type",
    values_to = "count"
  ) %>%
  mutate(
    record_type = factor(record_type, 
                        levels = c("filtered_records", "retained_records"),
                        labels = c("Filtered Records", "Retained Records")),

    count_capped = ifelse(count > 50, 50, count)
  ) %>%
  filter(!is.na(jurisdiction))

# Create the stacked bar plot
stacked_bar <- ggplot(stacked_bar_data, 
                     aes(x = year, y = count_capped, fill = record_type)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  scale_fill_manual(
    name = "Record Type",
    values = c("Retained Records" = "gray70", 
               "Filtered Records" = "red"),
    labels = c("Retained Records", "Filtered Records")
  ) +
  scale_x_continuous(
  ...
  labs(
    x = "Year",
    y = "Number of Snapshots",
    title = "Temporal Distribution of NDIS Data Snapshots",
    subtitle = "Showing retained vs filtered records across jurisdictions"
  ) +
  theme_minimal(base_size = 10) +
  theme(
  ...
  # Facet by jurisdiction for better readability
...
```{r}
#| label: stackedbar-coverage-aggregated
#| echo: true
#| code-fold: true
#| code-summary: "Show aggregated stacked bar chart code"
#| fig-width: 8
#| fig-height: 6

# Prepare data for aggregated stacked bar chart with labels
stacked_bar_aggregated <- ndis_data %>%
  mutate(year = year(capture_datetime)) %>%
  count(year, name = "total_records") %>%
  left_join(
    ndis_clean %>%
      mutate(year = year(capture_datetime)) %>%
      count(year, name = "retained_records"),
    by = "year"
  ) %>%
  mutate(
    retained_records = replace_na(retained_records, 0),
...
stacked_bar_labels <- stacked_bar_aggregated %>%
  group_by(year) %>%
  mutate(
...
stacked_bar_plot <- ggplot(stacked_bar_aggregated, aes(x = year, y = count, fill = record_type)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  geom_label(
  data = stacked_bar_labels %>%
    mutate(
      y_nudge = case_when(
        year == 2001 & record_type == "Filtered Records" ~ 240,
        year == 2002 & record_type == "Filtered Records" ~ 340,
        year == 2003 & record_type == "Filtered Records" ~ 340,
        year == 2002 & record_type == "Retained Records" ~ 240,
        year == 2003 & record_type == "Retained Records" ~ 240,
        TRUE ~ 0
      )
    ),
  aes(y = cumulative + y_nudge, label = count),
  fill = "white",
  color = "black",
  size = 5,
  label.size = 0.2,
  ...
  theme(
    ...
    axis.title.y = element_text(size = 28, margin = margin(r = 10))
  )
...
literature_data <- tribble(
  ...
  "FBI Brochure", "2006-12-01", 3977435, NA, 160582, NA, 45364, "FBI (Dec 2006)",
  "FBI Brochure", "2008-12-01", 6399200, NA, 248943, NA, 81955, "FBI (Dec 2008)",
  "FBI Brochure", "2010-12-01", 8564705, NA, 351951, NA, 130317, "FBI (Dec 2010)",
  "FBI Brochure", "2012-12-01", 10086404, NA, 446689, NA, 190560, "FBI (Dec 2012)",
...
p_dna_profiles <- ggplot() +
  ...
  geom_point(data = literature_dna %>% filter(variable == "Total"),
             ...
             shape = 4, size = 4, stroke = 2, color = "black") +
  ...
  annotate("rect", xmin = legend_start_date, 
           xmax = legend_start_date + years(7), 
           ymin = max_dna * 0.79, ymax = max_dna, 
           fill = "white", color = "black", alpha = 0.9, linewidth = 0.3) +
  # Legend items
  annotate("point", 
...
p_investigations <- ggplot() +
  ...
  geom_point(data = literature_investigations,
             ...
             shape = 4, size = 5, stroke = 2, color = "#CC79A7") +
  ...
  annotate("rect", xmin = legend_start_date, 
           ...
           ymin = max_inv * 0.88, ymax = max_inv, 
           fill = "white", color = "black", alpha = 0.9, linewidth = 0.3) +
  # Legend items
  annotate("point", 
           x = legend_start_date + months(6), 
  ...
  annotate("point",
           x = legend_start_date + months(6),
           y = max_inv * 0.91,
           shape = 4, size = 3, stroke = 1.5, color = "#CC79A7") +
  annotate("text",
           x = legend_start_date + years(1),
           y = max_inv * 0.91,
           label = "Literature Citation",
...
cat(paste("✓ Created final version at:", final_path, "\n"))
