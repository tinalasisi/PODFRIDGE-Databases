...
# Set CRAN mirror to prevent installation errors
...
# Display data types for each column with example values
enhanced_glimpse <- function(df) {
  glimpse_data <- data.frame(
    ...
    Example_Value = sapply(df, function(x) {
      ...
      # Convert to character and clean problematic characters
      val_str <- as.character(first_val)
      val_str <- gsub("\\\\", "/", val_str)
      val_str <- gsub("_", " ", val_str)
      val_str <- gsub("#", "", val_str)
      val_str <- gsub("&", "and", val_str)
      
      # Truncate if too long
      if (nchar(val_str) > 50) {
        val_str <- paste0(substr(val_str, 1, 47), "...")
      }
...
# Show preview
enhanced_glimpse(annual_dna_combined)
